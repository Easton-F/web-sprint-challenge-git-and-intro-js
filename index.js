...
function removeArtist(artists, number) {
  if (number >= 0 && number < artists.length){
    artists.splice(number, 1);
  }
  return artists;
}

const updatedArtists = removeArtist(artists, 3);

console.log('task 5:', updatedArtists);
...
function addArtist(artists, name, years, genre, nationality, bio) {
  ...
  const newArtists = {name, years, genre, nationality, bio};
   artists.push(newArtists);
   return artists;
  }
const addedArtists = addArtist(artists, "Easton Fritsch", "2000 - current day", "Web Design", "Caucasian", "Easton Fritsch is currently in school for Fullstack Web Developement and spends most of his time playing videogames with his friends and watching TV with his wife Briaunna Fritsch.");

console.log('task 6:', addedArtists);
...
function lotsOfArt(artists) {
  const dedicatedArtists = [];
  for (let i = 0; i < artists.length; i++){
    if (artists[i].paintings > 100){
      dedicatedArtists.push(artists[i].name)
    }
  }
